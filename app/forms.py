...
def not_future(form, field):
    if field.data > date.today():
        raise ValidationError('date cannot be after today')


def not_child(form, field):
    today = date.today()
    age = today.year - field.data.year - ((today.month, today.day) < (field.data.month, field.data.day))
    if age < 18:
        raise ValidationError('user must be older than 18')
            
def no_num(form, field):
    for i in field.data:
        if i.isdigit():
...
class RegistrationForm(FlaskForm):
    ...
    fname = StringField('First name', validators=[DataRequired(), Length(min=2, max=20),no_num])
    mname = StringField('Middle name', validators=[DataRequired(), Length(min=2, max=20),no_num])
    lname = StringField('Last name', validators=[DataRequired(), Length(min=2, max=20),no_num])
    ...
    dob = DateField('Date of birth', validators=[DataRequired(),not_child])
...
class RepairmentForm(FlaskForm):
  # Person data
  nationID = StringField('National ID', validators=[DataRequired(), Length(10), Regexp('^[1-2][0-9]{9}$', message='Enter valid nation ID')])
  fname = StringField('First name', validators=[DataRequired(), Length(min=4, max=20),no_num])
  mname = StringField('Middle name', validators=[DataRequired(), Length(min=4, max=20),no_num])
  lname = StringField('Last name', validators=[DataRequired(), Length(min=4, max=20),no_num])
  gender = SelectField('Gender', choices =types, validators=[DataRequired()])
  phonenumber = StringField('Phone number', validators=[DataRequired(), Length(10), Regexp('^[0][5][0-9]{8}$', message="Phonenumber must be like 05**")])
  dob = DateField('Date of birth', validators=[DataRequired(),not_child])
  ...
  car_mark = SelectField('Company', choices=marks, validators=[DataRequired()])
  car_model = StringField('Model (name)', validators=[DataRequired(), Length(min=2, max=20)])
  ...
  rep_date = DateField('Date of repairment', validators=[DataRequired(),not_future])
